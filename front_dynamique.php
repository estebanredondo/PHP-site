                ...
                <div class="col-12 col-md-8">
                    ...
                    <div class="row ligne d-flex justify-content-around mb-5">
                        ...
                        if (isset ($_GET['id_categorie'])) {
                            ...
                            $requete=$pdo->prepare('SELECT * FROM article WHERE id_categorie = ' . $categorie . ' ORDER BY id_article LIMIT 6');
                            $requete->execute();
                            while ($donnee=$requete->fetch())
                            {
                                echo '<div class="col-12 col-md-4 mb-5 article">
                                    <a href="page.php?id_article=' . $donnee['id_article'] . '">
<div style="height: 400px; background-image: url(img/' . $donnee['photo'] . '); background-position: center; background-repeat: no-repeat; background-size: cover;">
</div>
<div class="texteart text-left bg-white">'
                                    . $donnee['titre'] .
                                    '<span class="piedart"><br />' . $donnee['date_article'] . '</span>
</div></a>
</div>';
                            }
                        }
                        else {
                            $requete=$pdo->prepare('SELECT * FROM article ORDER BY id_article LIMIT 6');
                            $requete->execute();
                            while ($donnee=$requete->fetch())
                            {
                                echo '<div class="col-12 col-md-4 mb-5 article">
